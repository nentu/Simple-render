...
PathTracer::PathTracer(Scene &s, Camera &c, int depth, int spp)
    : scene(s), camera(c), maxDepth(depth), samplesPerPixel(spp) {}

void PathTracer::setSamplesPerPixel(int spp) { samplesPerPixel = spp; }
void PathTracer::setMaxDepth(int depth) { maxDepth = depth; }

// Трассировка одного пути
Color PathTracer::tracePath(float ox, float oy, float oz,
...
Color PathTracer::sampleDirectLight(const Vec3 &point, const Vec3 &normal,
...
std::vector<Color> PathTracer::render()
{
